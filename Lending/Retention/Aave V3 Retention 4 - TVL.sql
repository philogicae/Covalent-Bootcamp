WITH aave AS (
    ...
    SELECT
        ...
        chain_name,
        sum(sum(token_amount/pow(10, num_decimals)*price_in_usd)) OVER (PARTITION BY chain_name ORDER BY signed_at) AS TVL
    FROM aave a
    ...
    GROUP BY signed_at, chain_name
)

SELECT * FROM aave_tvl 
